...
朋友提醒与本人提醒复用公众平台中的同一个“吃药提醒”模板 ID，但在系统内部仍使用独立语义键 `CAREGIVER_OVERDUE`，避免授权和任务互相混用。授权归接收消息的朋友本人所有，并绑定一段有效 `care_links` 关系。服药者和朋友双方都同意后，系统把检查时间设为计划服药时间加 30 分钟：

```text
已服                → 不发送，未消费授权顺延到下一个计划日
明确记录为未服       → 发送“{本人称呼}已记录今日未服，请联系”
仍无记录             → 发送“{本人称呼}仍未完成记录，请提醒”
```

发送前必须再次验证关系仍为 active、服药者仍允许提醒、grant 仍保留给当前 job。消息中的称呼来自服药者主动填写的 `ownerLabel`，不是从登录态静默读取的微信昵称；称呼最多取 8 个字符，完整 `thing` 字段不超过 20 个字符。消息不包含具体药名、方案类型、照片或历史记录。一次实际发送后授权被消费，朋友需要再次主动开启下一次提醒。
...
- 复用本人模板 ID，以独立 `CAREGIVER_OVERDUE` 授权和任务完成双账号真机测试。
