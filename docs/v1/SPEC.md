...
- 根据 21+7 周期计算每天是服药日还是停药日。
- 今日页显示周期位置、计划时间和当天记录状态。
- 服药日可以“确认已服”或“记录未服”。
- 保存服务端记录时间，不把手机本地时间作为事实时间。
- 日历展示服药日、停药日、已服、未服和逾期未记录。
- 首次保存方案时申请一次微信订阅消息。
...
版本区间采用左闭右开 `[effectiveFrom, effectiveTo)`：首次版本 `effectiveFrom = startDate`；旧版本的 `effectiveTo` 等于新版本的 `effectiveFrom`。只修改提醒时间时保留原周期 `startDate`；修改药板第一天时要求 `startDate <= effectiveFrom`，并在保存前预览次日起周期如何变化。今日页仍使用今天生效的旧版本，月历按每个日格分别选择覆盖该日期的版本，不能只读取当前 active version。

## 7. 21+7 周期算法

底层仍保留参数化字段，但 V1 页面固定写入：
...
```ts
function getPlanDay(startDate: LocalDate, targetDate: LocalDate) {
  ...
  const dayIndex = ((offset % 28) + 28) % 28

  return {
    status: dayIndex < 21 ? 'active' : 'break',
    cycleDay: dayIndex + 1,
  }
}
```
...
服务端不接受客户端提供的 `openId`、`ownerId` 或任意 occurrenceId 作为可信身份。今日 occurrence 由服务端按当前方案和日期计算。
...
V1 不使用照片、昵称、手机号、位置和系统日历，因此不申请这些权限。
...
2. 连续模拟 35 天，21+7 状态全部正确。
3. 每天只能有一条事实记录。
4. 微信授权拒绝不会造成方案或记录丢失。
5. 具备有效授权时，定时任务能在目标分钟附近调用微信发送接口。
6. 没有照片、监督、系统日历相关入口、字段、集合或权限申请。
7. iOS 和 Android 各完成一次完整真机闭环。
...
这些功能不得提前进入 V1 数据接口或页面，以免拖慢最小闭环验证。
